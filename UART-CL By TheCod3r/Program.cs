...
while (showMenu)
{
	showMenu = await MainMenu(appTitle, Uart.RegionMap);
}
#endregion
		...
		switch (System.Console.ReadLine())
		{
			...
			case "2":
				...
				if (!string.IsNullOrEmpty(pathToDump)) // If the pathToDump string isn't empty or null, we can try and work with it
				{
					...
					try
					{
						var reader = new BinaryReader(new FileStream(pathToDump, FileMode.Open));
					...
					ConsoleModelInfo = Uart.HexStringToString(variantValue);

					string region = "Unknown Region";
					if (ConsoleModelInfo != null && ConsoleModelInfo.Length >= 3)
					{
						string suffix = ConsoleModelInfo[^3..];
					...
					ModelInfo = Uart.HexStringToString(variantValue) + " - " + region;
					#endregion
					...
					if (serialValue != null)
					{
						ConsoleSerialNumber = Uart.HexStringToString(serialValue);
						ConsoleSerialNumber = Uart.HexStringToString(serialValue);
					...
					{
						MotherboardSerialNumber = Uart.HexStringToString(moboSerialValue);
					}
					else
					{
					...
					while (!confirmed)
					{
						...
						if (changeConfirmation == "yes")
						{
							...
							else
							{
								...
								try
								{
									byte[] find = Uart.ConvertHexStringToByteArray(Regex.Replace("22010101", "0x|[ ,]", string.Empty).Normalize().Trim());
									byte[] replace = Uart.ConvertHexStringToByteArray(Regex.Replace("22030101", "0x|[ ,]", string.Empty).Normalize().Trim());

									byte[] bytes = File.ReadAllBytes(pathToDump);
									foreach (int index in Uart.PatternAt(bytes, find))
									{
									...
									byte[] find2 = Uart.ConvertHexStringToByteArray(Regex.Replace("22020101", "0x|[ ,]", string.Empty).Normalize().Trim());
									byte[] replace2 = Uart.ConvertHexStringToByteArray(Regex.Replace("22030101", "0x|[ ,]", string.Empty).Normalize().Trim());

									foreach (int index in Uart.PatternAt(bytes, find2))
									{
										for (int i = index, replaceIndex = 0; i < bytes.Length && replaceIndex < replace.Length; i++, replaceIndex++)
										{
					...
					while (!confirmed)
					{
						...
						if (changeConfirmation == "yes")
						{
							...
							else
							{
								...
								{
									byte[] find = Uart.ConvertHexStringToByteArray(Regex.Replace("22010101", "0x|[ ,]", string.Empty).Normalize().Trim());
									byte[] replace = Uart.ConvertHexStringToByteArray(Regex.Replace("22020101", "0x|[ ,]", string.Empty).Normalize().Trim());
									...
									byte[] find2 = Uart.ConvertHexStringToByteArray(Regex.Replace("22030101", "0x|[ ,]", string.Empty).Normalize().Trim());
									byte[] replace2 = Uart.ConvertHexStringToByteArray(Regex.Replace("22020101", "0x|[ ,]", string.Empty).Normalize().Trim());
					...
					while (!confirmed)
					{
						...
						if (changeConfirmation == "yes")
						{
							...
							else
							{
								...
								try
								{
									byte[] find = Uart.ConvertHexStringToByteArray(Regex.Replace("22020101", "0x|[ ,]", string.Empty).Normalize().Trim());
									byte[] replace = Uart.ConvertHexStringToByteArray(Regex.Replace("22010101", "0x|[ ,]", string.Empty).Normalize().Trim());
									...
									byte[] find2 = Uart.ConvertHexStringToByteArray(Regex.Replace("22030101", "0x|[ ,]", string.Empty).Normalize().Trim());
									byte[] replace2 = Uart.ConvertHexStringToByteArray(Regex.Replace("22010101", "0x|[ ,]", string.Empty).Normalize().Trim());
						...
						while (!newSerialValid)
						{
							...
							else
							{
								try
								{
									...
									int index = Uart.PatternAt(existingFile, oldSerialBytes).FirstOrDefault();
			...
			case "7":
				if (!string.IsNullOrEmpty(pathToDump)) // If the pathToDump string isn't empty or null, we can try and work with it
				{
					...
					MotherboardSerial = Uart.HexStringToString(moboValue);
						...
						while (!isDone)
						{
							...
							else
							{
								...
								try
								{
									...
									byte[] find = Uart.ConvertHexStringToByteArray(Regex.Replace(oldSerialHex, "0x|[ ,]", string.Empty).Normalize().Trim());
									byte[] replace = Uart.ConvertHexStringToByteArray(Regex.Replace(newSerialHex, "0x|[ ,]", string.Empty).Normalize().Trim());
			...
			case "8":
				...
				if (!string.IsNullOrEmpty(pathToDump)) // If the pathToDump string isn't empty or null, we can try and work with it
				{
					...
					ConsoleModel = Uart.HexStringToString(variantValue);
						...
						while (!isDone)
						{
							...
							else
							{
								...
								try
								{
									...
									byte[] find = Uart.ConvertHexStringToByteArray(Regex.Replace(oldModelHex, "0x|[ ,]", string.Empty).Normalize().Trim());
									byte[] replace = Uart.ConvertHexStringToByteArray(Regex.Replace(newModelHex, "0x|[ ,]", string.Empty).Normalize().Trim());
...
#region Main
static async Task<bool> MainMenu(string appTitle, Dictionary<string, string> regionMap)
{
					...
					switch (split[0])
					{
						...
						case "OK":
							...
							else
							{
								...
								System.Console.ForegroundColor = ConsoleColor.Green;
								string errorResult = await database.ParseErrors(errorCode);
...
#warning Refactor this, database downloading is lazy evaluated now
			bool success = true; // DownloadDatabase("http://uartcodes.com/xml.php", "errorDB.xml");
